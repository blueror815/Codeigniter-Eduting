...
//		$profile = $this->where('forgotten_password_code', $code)->users()->row(); //pass the code to profile
	...
	public function register($email, $password, $additional_data = array())
	{
		...
		$additional_data['s_no'] = $this->max_student_no_new(date("Ymd", time()));
	...
	public function max_student_no_new($date)
	{
	   $query = "SELECT MAX(s_no)  s_no FROM user WHERE  s_no LIKE 'S" . $date . "%' ";
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', s_no, first_name, middle_name, last_name, pass, user_type, nickname')
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === $user->pass)
			{
				...
				{
					$this->remember_user($user->s_no);
				}

				$this->trigger_events(array('post_login', 'post_login_successful'));
				$this->set_message('login_successful' . $remember);
	...
	public function user($id = NULL)
	{
		...
		$id || $id = $this->session->userdata['snum'];


		
		$result = $this->db->get_where($this->tables['users'], array('s_no' => $id), 1);
	...
	**/
	 ...
	 * @author Ben Edmunds
	 **/
	public function update_last_login($snum)
	{
		...
		$this->db->update($this->tables['users'], array('last_login_date' => time()), array('s_no' => $snum));
	...
	public function remember_user($id)
	{
		...
		$salt = sha1($user[0]->pass);
		

		$this->db->update($this->tables['users'], array('remember_code' => $salt), array('s_no' => $id));
	...
	public function login_remembered_user()
	{
		...
		//check for valid data and expire time check
		...
		if ($query->num_rows() == 1)
		{
			...
			$this->update_last_login($user->s_no);
